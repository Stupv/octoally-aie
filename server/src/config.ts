...
(() => {
  ...
  if (existsSync(oldOpenflowDir) && !existsSync(newDir)) {
    try {
      ...
      console.warn(
        `[MIGRATE] Could not rename ~/.openflow → ~/.octoally:`,
        err,
      );
    }
  }
  ...
  if (existsSync(oldHivecommandDir) && !existsSync(newDir)) {
    try {
      ...
      console.warn(
        `[MIGRATE] Could not rename ~/.hivecommand → ~/.octoally:`,
        err,
      );
    }
  }
...
const wantDtach =
  (process.env.OCTOALLY_USE_DTACH || process.env.HIVECOMMAND_USE_DTACH) !==
  "false";
const hasDtach = binaryAvailable("dtach");
const wantTmux =
  (process.env.OCTOALLY_USE_TMUX || process.env.HIVECOMMAND_USE_TMUX) !==
  "false";
const hasTmux = binaryAvailable("tmux");

if (wantDtach && !hasDtach) {
  console.warn(
    "  dtach not found — falling back to direct mode. Install with: sudo apt install dtach",
  );
}
if (wantTmux && !hasTmux) {
  console.warn(
    "  tmux not found — plain terminals will use dtach/direct mode. Install with: sudo apt install tmux",
  );
}
...
export const config = {
  ...
  authToken:
    process.env.OCTOALLY_TOKEN ||
    process.env.HIVECOMMAND_TOKEN ||
    process.env.OPENFLOW_TOKEN ||
    null,
  dbPath:
    process.env.DB_PATH ||
    (() => {
