...
async function start() {
  ...
  try {
    writeFileSync("/tmp/octoally-timing.log", "");
  } catch {}
  ...
  app.post("/api/open-terminal", async (req, reply) => {
    ...
    if (isMac) {
      ...
      exec(
        "which gnome-terminal xfce4-terminal konsole alacritty kitty wezterm xterm",
        { timeout: 3000 },
        (_err, stdout) => {
  ...
  const GITHUB_RELEASES_URL =
    "https://api.github.com/repos/ai-genius-automations/octoally/releases?per_page=20";
    ...
    try {
      ...
      if (cached && now - cached.checkedAt < 300_000) {
        return {
          ...
          channel,
          updateAvailable:
            cached.version !== "" && cached.version !== serverVersion,
        };
      }
      ...
      const releases = ((await resp.json()) as GitHubRelease[]).filter(
        (r) => !r.draft,
      );
      ...
      const entry = {
        ...
        name: pick?.name || "",
        url: pick?.html_url || "",
        prerelease: pick?.prerelease || false,
        checkedAt: now,
      };
