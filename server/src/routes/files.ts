...
export const fileRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{
    ...
    if (!pathA || !pathB)
      return reply.status(400).send({ error: "pathA and pathB are required" });
    ...
    try {
      await stat(resolvedA);
    } catch {
      return reply.status(404).send({ error: `File not found: ${pathA}` });
    }
    try {
      await stat(resolvedB);
    } catch {
      return reply.status(404).send({ error: `File not found: ${pathB}` });
    }
    ...
    return new Promise((resolvePromise) => {
      exec(`code "${resolved}"`, (err) => {
        if (err) {
          reply
            .status(500)
            .send({ error: "Failed to open VS Code", details: err.message });
        } else {
