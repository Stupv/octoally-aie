...
export const agentRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/agent/capabilities", async () => ({
    ...
    version: "1.1.0",
    description:
      "Structured control layer for OctoAlly sessions. All session interaction MUST go through these APIs. The execute endpoint returns clean, readable output rendered through a virtual terminal — no ANSI artifacts, no TUI garbage.",
    critical: [
    ...
    stateMachine: {
      states: {
        ...
        waiting_for_input:
          "A prompt was detected. Check promptType and choices for what is expected.",
      },
      transitions: [
        {
          from: "busy",
          to: "idle",
          trigger:
            "No PTY output for quiescenceMs (default 2000ms) and no prompt pattern detected",
        },
        {
          from: "busy",
          to: "waiting_for_input",
          trigger:
            "No PTY output for quiescenceMs and a prompt pattern was detected in output tail",
        },
        { from: "idle", to: "busy", trigger: "New PTY output received" },
        {
          from: "waiting_for_input",
          to: "busy",
          trigger: "New PTY output received (typically after input is sent)",
        },
      ],
    },
    promptTypes: {
      choice:
        'Numbered option list (e.g. "1. Option A\\n2. Option B"). Respond with the number.',
    ...
    endpoints: [
      {
        ...
        description:
          "Rendered terminal output + inline state for polling. Returns clean text (no ANSI), with cursor-based incremental fetching. No side effects — safe for periodic polling.",
        request: {
          lines: {
            type: "number",
            required: false,
            default: 50,
            description:
              "Max lines of rendered terminal text to return (from the end). Max 5000.",
          },
          since: {
            type: "number",
            required: false,
            description:
              "Cursor value from a previous response. Only returns output produced after this point. Omit for initial fetch.",
          },
          cols: {
            type: "number",
            required: false,
            default: 120,
            description: "Terminal column width for rendering.",
          },
          rows: {
            type: "number",
            required: false,
            default: 40,
            description: "Terminal row height for rendering.",
          },
        },
        response: {
          sessionId: "string",
          ...
          cursor:
            "number | null — pass back as ?since= for incremental polling",
      ...
      },
      {
        ...
        description:
          "Cancel a stuck execute request. Returns { ok: true } if one was cancelled, { ok: false } if none pending.",
      },
      {
        method: "GET",
      ...
      },
      {
        ...
        description:
          "Send input and get clean rendered output. Output is processed through a virtual terminal that handles all TUI cursor movements, screen redraws, and ANSI codes — you get readable text, not raw terminal data. Only one execute per session at a time (409 if busy).",
        request: {
          input: {
            type: "string",
            required: true,
            description:
              "Text to send (carriage return appended automatically)",
          },
          waitFor: {
            type: "string",
            required: false,
            description: "Regex pattern — resolve early when matched in output",
          },
          timeout: {
            type: "number",
            required: false,
            default: 30000,
            description:
              'Max ms to wait before returning with status "timeout"',
          },
          stripAnsi: {
            type: "boolean",
            required: false,
            default: true,
            description:
              "Render through virtual terminal for clean output (default true, always use true)",
          },
          quiescenceMs: {
            type: "number",
            required: false,
            default: 2000,
            description: "Ms of silence before considering output complete",
          },
        },
        response: {
          ...
          output:
            "string — clean rendered text showing what changed on screen since input was sent",
          durationMs: "number",
      ...
      },
      {
        ...
        description:
          "Real-time structured WebSocket. Receives state changes and clean output. Can send execute requests.",
        incomingMessages: [
          {
            type: "execute",
            fields: {
        ...
        outgoingMessages: [
          {
            type: "connected",
            fields: { sessionId: "string", state: "SessionState" },
            description: "Sent on connection with current state.",
          },
          {
            type: "state_change",
            fields: {
            ...
            description: "Sent when state transitions.",
          },
          {
            type: "output",
            fields: { text: "string" },
            description: "ANSI-stripped output chunks as they arrive.",
          },
          {
            type: "execute_result",
            fields: {
            ...
            description: "Result of an execute request.",
          },
          {
            type: "state",
            fields: { requestId: "string", "...": "SessionState fields" },
    ...
    operationalGuidance: {
      responsiveness: {
        description:
          "Agents controlling OctoAlly sessions should run a periodic health check to avoid getting stuck on long-running commands or unresponsive sessions.",
        recommendation:
          "Run a 30-second cron/interval that polls GET /api/sessions to check for stuck sessions and GET /api/sessions/:id/state for any session you are actively managing.",
        checkLogic: [
      ...
      execTimeouts: {
        description:
          "Always set explicit timeouts on execute calls. Long-running shell commands inside ruflo can hang indefinitely.",
        defaults: { timeout: 30000, quiescenceMs: 2000 },
        forSlowCommands: { timeout: 120000, quiescenceMs: 5000 },
      },
      crashRecovery: {
        description:
          "OctoAlly automatically resumes crashed sessions that have a captured Claude session UUID. On server restart, sessions with a claude_session_id are re-spawned and sent /resume <uuid> to restore context.",
        agentAction:
          "After a server restart, poll GET /api/sessions?status=running to discover auto-resumed sessions. Your session IDs remain stable.",
      },
    },
  }));
  ...
  app.post<{
    ...
    if (tracker.hasPendingExecute) {
      return reply
        .status(409)
        .send({ error: "Session already processing an execute request" });
    }
    ...
    if (typeof input !== "string") {
      return reply
        .status(400)
        .send({ error: "input is required and must be a string" });
    }
  ...
  app.get<{
    ...
    const rendered = await new Promise<string>((resolve) => {
      ...
      function finalize() {
        ...
        const trimmed =
          collapsed.length > lines ? collapsed.slice(-lines) : collapsed;
    ...
    const tracker = getTracker(id);
    const processState = tracker
      ? tracker.state.processState
      ...
      try {
        ...
        switch (msg.type) {
          case "execute": {
            ...
            if (msg.waitFor) {
              try {
                waitFor = new RegExp(msg.waitFor);
              } catch {
                socket.send(
                  JSON.stringify({
                    type: "execute_result",
