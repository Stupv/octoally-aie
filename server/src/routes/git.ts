...
export const gitRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get<{ Querystring: { path: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        const [branchOut, statusOut, aheadBehindOut, remoteOut] =
          await Promise.all([
  ...
  app.get<{ Querystring: { path: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const current = (
          await git(path, ["rev-parse", "--abbrev-ref", "HEAD"])
        ).trim();
  ...
  app.post<{ Body: { path: string; branch: string; isRemote?: boolean } }>(
    "/git/checkout",
    async (req, reply) => {
  ...
  app.get("/git/gh-accounts", async (_req, reply) => {
    try {
      ...
      const { stdout: userOut } = await execFileAsync(
        "gh",
        ["api", "/user", "--jq", ".login"],
        { maxBuffer: 1024 * 1024 },
      );
  ...
  }>("/git/create-repo", async (req, reply) => {
    const {
      path,
      name,
      owner,
