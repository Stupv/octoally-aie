...
export function parseSplitRows(raw: string): SplitRow[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("-")) {
      ...
      while (j < lines.length && lines[j].startsWith("+")) {
        added.push(lines[j].slice(1));
        j++;
      }
      const max = Math.max(removed.length, added.length);
      for (let k = 0; k < max; k++) {
        const hasL = k < removed.length,
          hasR = k < added.length;
        result.push({
          leftNum: hasL ? ++lNum : null,
          leftText: hasL ? removed[k] : "",
          leftType: hasL ? "removed" : "normal",
          rightNum: hasR ? ++rNum : null,
          rightText: hasR ? added[k] : "",
          rightType: hasR ? "added" : "normal",
          hunkIndex: currentHunkIdx,
        });
      }
      i = j - 1;
...
export function lineStyle(line: string) {
  if (line.startsWith("+") && !line.startsWith("+++"))
    return { color: "var(--success)", bg: "rgba(63,185,80,0.08)" };
  if (line.startsWith("-") && !line.startsWith("---"))
    return { color: "var(--error)", bg: "rgba(248,81,73,0.08)" };
  if (line.startsWith("@@"))
    return { color: "var(--accent)", bg: "transparent" };
  if (
    line.startsWith("index ") ||
    line.startsWith("---") ||
    line.startsWith("+++")
...
export function FileSeparator({ name }: { name: string }) {
  return (
    <div
      ...
      style={{ background: "var(--accent)", color: "#fff", minHeight: 28 }}
    >
...
export const SplitHalf = forwardRef<
  HTMLDivElement,
  {
    num: number | null;
    text: string;
...
>(function SplitHalf(
  ...
  if (type === "separator") {
    return (
      <div
        ref={ref}
        className="flex items-center px-2 text-xs font-semibold"
        style={{
          height: ROW_H + 8,
  ...
  if (type === "header") {
    return (
      <div
        ref={ref}
        className="text-xs leading-5 px-1"
        style={{
          height: ROW_H,
          color: "var(--accent)",
  ...
  return (
    <div
      ...
      style={{
        height: ROW_H,
        background: bg,
        outline: isCurrentHunk ? `1px solid rgba(234,179,8,0.15)` : undefined,
      }}
    >
      <span
        className="shrink-0 text-right pr-1.5 select-none leading-5"
        style={{
          width: GUTTER_W,
      ...
      <span
        className="shrink-0 w-4 text-center select-none leading-5"
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (scrollHeight <= clientHeight) {
        setViewportRatio({ top: 0, height: 1 });
        return;
      }
      setViewportRatio({
      ...
      >
        <div style={{ minWidth: "fit-content" }}>
          {lines.map((line, i) => {
            ...
            const isReverted =
              hunkIdx !== undefined && revertedHunks.has(hunkIdx);
            ...
            return (
              <div
                ...
                style={{
                  ...
                  whiteSpace: "pre",
                  cursor: isHunkHeader ? "pointer" : undefined,
                }}
                onClick={
                  isHunkHeader && hunkIdx !== undefined
                    ? () => onHunkClick(hunkIdx)
                    : undefined
                }
              >
...
export function SplitDiff({
  ...
  onHunkClick: (idx: number) => void;
  isEditable: boolean;
  onLineEdit: (lineNum: number, text: string) => void;
    ...
    <div className="flex-1 min-h-0 flex">
      ...
      >
        <div style={{ minWidth: "fit-content" }}>
          {rows.map((r, i) => {
            const isReverted =
              r.hunkIndex !== null && revertedHunks.has(r.hunkIndex);
            const isHunkHeader =
            ...
            return (
              <SplitHalf
                ...
                isEditable={false}
                onClick={
                  isHunkHeader && r.hunkIndex !== null
                    ? () => onHunkClick(r.hunkIndex!)
                    : undefined
                }
              />
            );
      ...
      >
        <div style={{ minWidth: "fit-content" }}>
          ...
          {rows.map((r, i) => {
            ...
            const edited =
              r.rightNum !== null ? editedLines.get(r.rightNum) : undefined;
            return (
              <SplitHalf
                key={i}
                num={r.rightNum}
                ...
                onEdit={
                  r.rightNum !== null
                    ? (text) => onLineEdit(r.rightNum!, text)
                    : undefined
                }
                onClick={
                  r.rightType === "header" &&
