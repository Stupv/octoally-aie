...
function ProjectForm({
  ...
  const [defaultWebUrl, setDefaultWebUrl] = useState(
    project?.default_web_url || "",
  );
  ...
  useEffect(() => {
    ...
    claudeMdQuery.isSuccess,
    claudeMdQuery.isError,
    claudeMdQuery.data?.content,
  ]);
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      ...
      if (defaultWebUrl !== (project!.default_web_url || ""))
        fields.default_web_url = defaultWebUrl || null;
    ...
    onSuccess: async () => {
      ...
      try {
        if (claudeMd !== initialClaudeMd)
          await api.files.write(`${savePath}/CLAUDE.md`, claudeMd);
        if (settingsJson !== initialSettingsJson)
          await api.files.write(
            `${savePath}/.claude/settings.json`,
            settingsJson,
          );
      } catch {
  ...
  const handleSave = async () => {
    ...
    if (installRuflo && path) {
      try {
        ...
        try {
          await api.files.read(`${path}/CLAUDE.md`);
          conflicts.claudeMd = true;
        } catch {}
        try {
          await api.files.read(`${path}/.claude/settings.json`);
          conflicts.settingsJson = true;
        } catch {}
  ...
  const filesLoading =
    mode === "edit" &&
    !!projectPath &&
    (claudeMdQuery.isLoading || settingsQuery.isLoading);

  const [installRuflo, setInstallRuflo] = useState(true);
  const [rufloConflicts, setRufloConflicts] = useState<{
    settingsJson: boolean;
    claudeMd: boolean;
  } | null>(null);
  const [rufloConfirmPending, setRufloConfirmPending] = useState(false);
  const [rufloSetupStatus, setRufloSetupStatus] = useState<
      ...
      <div className="mx-auto" style={{ maxWidth: "720px" }}>
        ...
        >
          <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setPath(val);
                      if (!name || name === prevFolderName(path))
                        setName(folderName);
                    }}
            ...
            >
              ...
              {createRepo && (
                ...
                >
                  <div>
                    <label
                      className="text-[10px] font-semibold uppercase mb-1 block"
                      style={{ color: "var(--text-secondary)" }}
                    >
                      Account
                    </label>
                  ...
                  <button
                    onClick={async () => {
                      if (!projectPath || repoCreating || !repoName.trim())
                        return;
                    ...
                    style={{
                      ...
                      color: "#fff",
                      opacity:
                        !repoName.trim() || ghAccounts.length === 0 ? 0.5 : 1,
                    }}
                  >
                    {repoCreating ? (
                      <>
            ...
            {mode === "edit" && (
              <div
                className="grid grid-cols-3 gap-x-6 gap-y-4 items-stretch pt-2"
                style={{ borderTop: "1px solid var(--border)" }}
              >
                {/* Prompts */}
                <div className="flex flex-col gap-4">
...
function GitInfoBadge({ projectPath }: { projectPath: string }) {
  ...
  const localBranches = (branchData?.branches || []).filter(
    (b) => !b.remote && b.name !== gitStatus.branch,
  );
  const remoteBranches = (branchData?.branches || []).filter(
    (b) => b.remote && b.name !== `origin/${gitStatus.branch}`,
  );
                  ...
                  <>
                    ...
                    {localBranches.map((b) => (
                      <button
                        key={b.name}
                        onClick={() => {
                          setSwitchTarget({ name: b.name, isRemote: false });
                          setDropdownOpen(false);
                        }}
                  ...
                  <>
                    ...
                    {remoteBranches.map((b) => (
                      <button
                        key={b.name}
                        onClick={() => {
                          setSwitchTarget({ name: b.name, isRemote: true });
                          setDropdownOpen(false);
                        }}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] w-full text-left hover:bg-white/5"
                        style={{ color: "var(--text-primary)" }}
        ...
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              alert(
                "Branch switch failed: " + (err.message || "Unknown error"),
              );
            } finally {
              setSwitching(false);
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-md text-xs font-medium"
            style={{
              background: creating ? "#3b82f680" : "#3b82f6",
              color: "#fff",
              border: "none",
              opacity: !repoName.trim() || accounts.length === 0 ? 0.5 : 1,
            }}
          >
...
export function ProjectDashboard({ onOpenProject }: ProjectDashboardProps) {
  ...
  const [rufloConfirm, setRufloConfirm] = useState<{
    id: string;
    conflicts: { settingsJson: boolean; claudeMd: boolean };
  } | null>(null);
    ...
    onSuccess: async (_data, id) => {
      ...
      if (dctxStatusData?.globalInstalled && dctxStatus?.installed) {
        try {
          ...
          /* DevCortex reinstall failed — ruflo re-init still succeeded */
        }
      }
    },
  ...
  const activeSessionsByProject = useMemo(() => {
    const map: Record<
      string,
      { hivemind: number; terminal: number; agent: number; total: number }
    > = {};
    for (const s of sessionsData?.sessions || []) {
      if (
        ...
        if (!map[s.project_id])
          map[s.project_id] = { hivemind: 0, terminal: 0, agent: 0, total: 0 };
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        >
          ...
          >
            ...
            <p className="text-xs" style={{ color: "var(--text-secondary)" }}>
              ...
              <a
                href="https://aigeniusautomations.com"
                target="_blank"
                rel="noopener noreferrer"
      ...
      <div
        className="mx-auto w-full flex flex-col h-full"
          ...
          {loadingProjects ? (
            ...
            <div className="flex flex-col gap-6">
              {(() => {
                ...
                const devcortexProjects = projects.filter(
                  (p) => dctxGlobal && dctxStatuses[p.id]?.installed,
                );
                const rufloOnlyProjects = projects.filter(
                  (p) =>
                    cfStatuses[p.id]?.installed &&
                    !(dctxGlobal && dctxStatuses[p.id]?.installed),
                );
                ...
                if (devcortexProjects.length > 0)
                  groups.push({
                    label: "DevCortex Projects",
                    color: "#a855f7",
                    icon: Brain,
                    items: devcortexProjects,
                  });
                if (rufloOnlyProjects.length > 0)
                  groups.push({
                    label: "RuFlo Projects",
                    color: "#60a5fa",
                    icon: Zap,
                    items: rufloOnlyProjects,
                  });
                        ...
                        return (
                          ...
                          >
                            ...
                            {(() => {
                              const hasDctx =
                                dctxGlobal &&
                                dctxStatuses[project.id]?.installed;
                              ...
                              const version = hasDctx
                                ? dctxStatuses[project.id]?.version
                                : cfStatus?.version;
                              return (
                                <div
                                ...
                                >
                                  ...
                                  {(() => {
                                    const counts =
                                      activeSessionsByProject[project.id];
                                    if (!counts || counts.total === 0)
                                      return null;
                                    const parts: string[] = [];
                                    if (counts.hivemind > 0)
                                      parts.push(`${counts.hivemind} hive`);
                                    if (counts.agent > 0)
                                      parts.push(`${counts.agent} agent`);
                                    if (counts.terminal > 0)
                                      parts.push(`${counts.terminal} term`);
                                    const badgeBg =
                                      counts.total <= 2
                                        ? "#22c55e20"
                                        : counts.total <= 4
                              ...
                              >
                                ...
                                {!cfStatus?.installed && (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      color: isInstalling
                                        ? "#facc15"
                                        : installError && installingId === null
                                          ? "#f87171"
                                          : "#60a5fa",
                                    }}
                                    title={
                                      isInstalling
                                        ? "Installing RuFlo..."
                                        : "Install RuFlo (memory + hive-mind)"
                                    }
                                  >
                                    {isInstalling ? (
                                      <>
                                ...
                                {cfStatus?.installed && (
                                  <button
                                    ...
                                    title={
                                      isInstalling
                                        ? "Re-initializing RuFlo..."
                                        : "Re-init RuFlo (update agents + memory)"
                                    }
                                  >
                                    {isInstalling ? (
                                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
