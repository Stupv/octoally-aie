...
function categorizeFiles(files: GitFileStatus[]) {
  const staged: CategorizedFile[] = [],
    changed: CategorizedFile[] = [],
    untracked: CategorizedFile[] = [];
  for (const file of files) {
    if (file.x !== " " && file.x !== "?" && file.x !== "!")
      staged.push({ file, statusChar: file.x });
    if (file.y !== " " && file.y !== "?" && file.y !== "!")
      changed.push({ file, statusChar: file.y });
    if (file.x === "?" && file.y === "?")
      untracked.push({ file, statusChar: "?" });
  }
...
function statusColor(s: string) {
  switch (s) {
    ...
    case "?":
      return "var(--text-tertiary, #6e7681)";
    default:
...
export function GitPanel({
  ...
  useEffect(() => {
    if (isVisible && hasLoadedOnce.current) refresh();
  }, [isVisible, refresh]);
  ...
  const handleStage = async (fps: string[]) => {
    await api.git.stage(projectPath, fps);
  ...
  const handleUnstage = async (fps: string[]) => {
    await api.git.unstage(projectPath, fps);
    refresh();
  };
  const handleDiscard = async (fps: string[]) => {
    await api.git.discard(projectPath, fps);
    setSelectedFiles((prev) => {
      const next = new Set(prev);
      fps.forEach((f) => next.delete(f));
      return next;
  ...
  const selectedUnstagedPaths = [...selectedFiles].filter(
    (p) =>
      changed.some((f) => f.file.path === p) ||
      untracked.some((f) => f.file.path === p),
  );
  ...
  const handleCommit = async () => {
    ...
    try {
      if (staged.length === 0) {
        const allPaths = [
          ...changed.map((f) => f.file.path),
          ...untracked.map((f) => f.file.path),
        ];
  ...
  if (error && !branch)
    return (
      ...
      >
        <div>
          <p style={{ color: "var(--error)" }}>Not a git repository</p>
          <p className="text-xs mt-1">
            Initialize a git repo in this project to use source control.
          </p>
        </div>
      </div>
    );
  ...
  const displayDiff =
    detail?.type === "commit"
      ? selectedCommitFile
        ? filterDiffToFile(commitDiff, selectedCommitFile)
        : commitDiff
      : diffContent;
  ...
  const editFilePath =
    detail?.type === "diff" ? `${projectPath}/${detail.path}` : undefined;
    ...
    <div ref={containerRef} className="h-full flex">
      ...
      {!leftCollapsed && (
        <div
          ...
          style={{ width: leftPanelWidth, background: "var(--bg-primary)" }}
        >
          {/* Header */}
          <div
          ...
          >
            ...
            >
              <button
                ...
                className="flex items-center gap-1 px-1 py-0.5 rounded hover:bg-white/10 transition-colors"
              ...
              {branchPickerOpen && (
                <div
                  className="absolute top-full left-0 mt-1 z-50 rounded shadow-lg overflow-hidden"
                  style={{
                    background: "var(--bg-secondary)",
                  ...
                  <div className="overflow-y-auto" style={{ maxHeight: 280 }}>
                    {(() => {
                      ...
                      const localBranches = branches.filter(
                        (b) =>
                          !b.remote && b.name.toLowerCase().includes(filter),
                      );
                      ...
                      const remoteBranches = branches.filter(
                        (b) =>
                          ...
                          !localNames.has(b.name.replace(/^origin\//, "")),
                      );
                      return (
                        <>
                            ...
                            <>
                              ...
                              {remoteBranches.map((b) => (
                                <button
                                  key={b.name}
                                  onClick={() =>
                                    handleSwitchBranch(b.name, true)
                                  }
                                  className="flex items-center gap-2 w-full px-3 py-1.5 text-xs text-left hover:bg-[var(--bg-tertiary)] transition-colors"
                                  style={{ color: "var(--text-primary)" }}
                                >
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-0.5 px-2 py-1 text-xs rounded font-medium transition-colors shrink-0"
              style={{
                background: ahead > 0 ? "var(--bg-tertiary)" : "transparent",
                color:
                  ahead > 0 ? "var(--text-secondary)" : "var(--text-tertiary)",
                opacity: ahead === 0 ? 0.4 : 1,
              }}
              title="Push to remote"
            >
              <ArrowUp className="w-3 h-3" />
              {ahead > 0 && <span className="opacity-70">{ahead}</span>}
            </button>
            {behind > 0 && (
              <button
                onClick={handlePull}
              ...
              >
                ...
                <span className="opacity-70">{behind}</span>
              </button>
            )}
          </div>
          {error && (
          ...
          {selectedUnstagedPaths.length > 0 && (
            ...
            >
              <span
                ...
                style={{ color: "var(--text-secondary)" }}
              >
                {selectedUnstagedPaths.length} selected
              </span>
              <button
                onClick={() => {
                  handleStage(selectedUnstagedPaths);
                }}
              ...
              <button
                onClick={() => {
                  handleDiscard(selectedUnstagedPaths);
                }}
                className="px-2 py-0.5 text-xs rounded font-medium transition-colors"
                style={{
                  background: "var(--bg-tertiary)",
            ...
            {staged.length > 0 && (
              <FileSection
                ...
                headerActions={
                  <button
                    onClick={() =>
                      handleUnstage(staged.map((f) => f.file.path))
                    }
                    title="Unstage all"
              ...
              >
                {staged.map((f) => (
                  <FileRow
                    ...
                    isActive={
                      detail?.type === "diff" &&
                      detail.path === f.file.path &&
                      detail.staged
                    }
                    onClick={() => handleViewDiff(f.file.path, true)}
                    actions={
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleUnstage([f.file.path]);
                        }}
                        title="Unstage"
                        className="p-0.5 rounded hover:bg-white/10"
                        style={{ color: "var(--text-secondary)" }}
              ...
              <FileSection
                ...
                headerActions={
                  <>
                    <button
                      onClick={() =>
                        handleStage(changed.map((f) => f.file.path))
                      }
                      title="Stage all"
                      className="p-0.5 rounded hover:bg-white/10"
                      style={{ color: "var(--text-secondary)" }}
                    ...
                    <button
                      onClick={() =>
                        handleDiscard(changed.map((f) => f.file.path))
                      }
                      title="Discard all"
                      className="p-0.5 rounded hover:bg-white/10"
                      style={{ color: "var(--text-secondary)" }}
              ...
              >
                {changed.map((f) => (
                  <FileRow
                    ...
                    onClick={() => handleViewDiff(f.file.path, false)}
                    selected={selectedFiles.has(f.file.path)}
                    onSelect={() => toggleFileSelect(f.file.path)}
                    actions={
                      <>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleStage([f.file.path]);
                          }}
                          title="Stage"
                          className="p-0.5 rounded hover:bg-white/10"
                          style={{ color: "var(--text-secondary)" }}
            ...
            {untracked.length > 0 && (
              <FileSection
                ...
                headerActions={
                  <button
                    onClick={() =>
                      handleStage(untracked.map((f) => f.file.path))
                    }
                    title="Stage all"
                    className="p-0.5 rounded hover:bg-white/10"
                    style={{ color: "var(--text-secondary)" }}
          ...
          <div
            ...
            style={{
              ...
              maxHeight: commitsOpen ? "50%" : "auto",
            }}
          >
            <button
              onClick={() => setCommitsOpen(!commitsOpen)}
              className="flex items-center w-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wider shrink-0"
              style={{
                color: "var(--text-secondary)",
                background: "var(--bg-secondary)",
              }}
            >
              ...
              Commits{" "}
              <span className="ml-1 opacity-60">({commits.length})</span>
            </button>
            {commitsOpen && (
              <div className="flex-1 min-h-0 overflow-y-auto">
                {commits.map((c) => (
                  <div
                    key={c.hash}
                    onClick={() => handleViewCommit(c)}
                    className="flex items-start gap-2 px-3 py-1.5 text-xs cursor-pointer hover:bg-[var(--bg-tertiary)] transition-colors"
                    style={{
                      borderBottom: "1px solid var(--border)",
                      background:
                        detail?.type === "commit" && detail.hash === c.hash
                          ? "var(--bg-tertiary)"
                          : "transparent",
                    }}
                  >
      ...
      {!leftCollapsed && (
        <div
          ...
          onMouseEnter={(e) =>
            (e.currentTarget.style.background = "var(--accent, #4a9eff)")
          }
          onMouseLeave={(e) => {
            if (!isDraggingRef.current)
          ...
          >
            {leftCollapsed ? (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded transition-colors hover:bg-[var(--bg-tertiary)]"
                style={{ color: "var(--text-secondary)" }}
              >
          ...
          <>
            ...
            >
              <div className="flex items-center gap-1.5 min-w-0">
                ...
                >
                  {detail.type === "diff"
                    ? `${detail.path}${detail.staged ? " (staged)" : ""}`
                    : `${detail.hash.slice(0, 7)} — ${detail.message}`}
                </span>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                ...
                <ToggleBtn
                  active={fullFile}
                  onClick={() => setFullFile(!fullFile)}
                  title={fullFile ? "Full file context" : "Diff hunks only"}
                >
            ...
            {detail.type === "commit" && commitFiles.length > 1 && (
              ...
              >
                <button
                  ...
                  style={{
                    background: !selectedCommitFile
                      ? "var(--accent)"
                      : "var(--bg-tertiary)",
                    color: !selectedCommitFile
                      ? "#fff"
                      : "var(--text-secondary)",
                  }}
                >
  ...
  if (!diff)
    return (
      <div
        ...
        style={{ color: "var(--text-tertiary)" }}
      >
        No diff available (new or binary file?)
      </div>
    );
        ...
        >
          ...
          {isEditable && (
            <>
              ...
              <button
                onClick={handleUndo}
                disabled={hunks.length === 0 || revertedHunks.has(currentHunk)}
              ...
              <button
                ...
                className="flex items-center gap-1 px-1.5 py-0.5 rounded text-xs hover:bg-white/10 disabled:opacity-30 transition-colors"
                style={{ color: "var(--text-secondary)" }}
                title="Reapply last undone change (redo)"
              >
...
function FileSection({
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--border)" }}>
      <div
        onClick={onToggle}
        className="flex items-center w-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wider group cursor-pointer"
        style={{
          color: "var(--text-secondary)",
          background: "var(--bg-secondary)",
        }}
...
function FileRow({
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center gap-1.5 px-3 py-1 text-xs cursor-pointer group transition-colors hover:bg-[var(--bg-tertiary)]"
      style={{
        background: isActive
          ? "var(--bg-tertiary)"
          : selected
    ...
    >
      {onSelect && (
        <input
          ...
          onClick={(e) => e.stopPropagation()}
          className="shrink-0 w-3.5 h-3.5 rounded accent-[var(--accent)]"
