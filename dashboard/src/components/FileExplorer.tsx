...
export function FileExplorer({
  ...
  const [initialState] = useState(() =>
    instanceId ? loadExplorerState(instanceId) : null,
  );
  const [currentPath, setCurrentPath] = useState(
    initialState?.currentPath ?? rootPath,
  );
  ...
  const isDirty = activeTab
    ? activeTab.editedContent !== activeTab.content
    : false;
  ...
  async function revealFileInTree(filePath: string) {
    ...
    setTimeout(() => {
      const el = treeScrollRef.current?.querySelector(
        `[data-filepath="${CSS.escape(filePath)}"]`,
      );
  ...
  useEffect(() => {
    ...
    revealFileInTree(activeTabPath).finally(() => {
  ...
  async function selectForCompare(path: string) {
    ...
    try {
      ...
      const leftContent =
        leftTab?.editedContent ?? (await api.files.read(leftPath)).content;
      const rightContent =
        rightTab?.editedContent ?? (await api.files.read(rightPath)).content;
  ...
  function buildEffectiveContent(side: "left" | "right"): string {
    ...
    let lines = (
      isLeft ? compareState.leftContent : compareState.rightContent
    ).split("\n");
  ...
  async function saveCompareChanges() {
    ...
    try {
      ...
      const leftChanged = [...compareState.appliedHunks.values()].some(
        (d) => d === "right",
      );
      // 'left' arrow = keep left → right file changes
      const rightChanged = [...compareState.appliedHunks.values()].some(
        (d) => d === "left",
      );
      ...
      if (leftChanged) {
        ...
        if (leftTab)
          updateTab(compareState.leftPath, {
            content: newContent,
      ...
      if (rightChanged) {
        ...
        if (rightTab)
          updateTab(compareState.rightPath, {
            content: newContent,
            editedContent: newContent,
          });
      ...
      const { diff } = await api.files.diff(
        compareState.leftPath,
        compareState.rightPath,
      );
      const hunks = parseHunks(diff);
      const leftContent = (await api.files.read(compareState.leftPath)).content;
      const rightContent = (await api.files.read(compareState.rightPath))
        .content;
      setCompareState({
          ...
          setTabs([
            {
              path: fileData.path,
              name:
                savedState.selectedFile.split("/").pop() ||
        ...
        >
          ...
          <button
            ...
            style={{
              width: 24,
              height: 24,
              color: showHidden ? "var(--accent)" : "var(--text-tertiary)",
            }}
          ...
          >
            {tabs.map((tab) => {
              ...
              const isCompareTarget =
                compareSelecting &&
                compareSelecting !== "waiting" &&
                compareSelecting !== tab.path;
                    ...
                    title={
                      compareSelecting
                        ? isCompareSelected
                          ...
                          : isCompareTarget
                            ? `Compare with ${compareSelecting.split("/").pop()}`
                            : "Click to select"
                        : tab.path
                    }
                  >
            ...
            {tabs.length >= 2 && (
              ...
              >
                ...
                {compareSelecting && compareSelecting !== "waiting" && (
                  <span>Pick 2nd file</span>
                )}
              </button>
            )}
          </div>
                ...
                {isMarkdown && (
                  <button
                    onClick={() =>
                      updateTab(activeTab.path, {
                        viewMode:
                          activeTab.viewMode === "preview" ? "edit" : "preview",
                      })
                    }
                    className="flex items-center gap-1 px-2 py-0.5 rounded text-xs transition-colors"
                    style={{
            ...
            {isMarkdown && activeTab.viewMode === "preview" ? (
              ...
              <div className="flex-1 overflow-auto">
                <CodeMirror
                  ...
                  onChange={(value) =>
                    updateTab(activeTab.path, { editedContent: value })
                  }
...
function FileCompareView({
  ...
  const rows = useMemo(
    () => parseSplitRows(compareState.diff),
    [compareState.diff],
  );
  ...
  useEffect(() => {
    ...
    if (el && leftRef.current) {
      ...
      if (gutterRef.current)
        gutterRef.current.scrollTop = leftRef.current.scrollTop;
    }
  }, [currentHunk]);
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      >
        <div className="flex items-center gap-3">
          <span>
            <strong style={{ color: "var(--error)" }}>{leftName}</strong>
          </span>
          <span>vs</span>
          <span>
            <strong style={{ color: "var(--success)" }}>{rightName}</strong>
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          {saveMessage && (
            <span
              style={{
                color:
                  saveMessage === "Saved" ? "var(--success)" : "var(--error)",
              }}
            >
              {saveMessage}
            </span>
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        >
          <div style={{ minWidth: "fit-content" }}>
            ...
            {rows.map((r, i) => {
              const applied =
                r.hunkIndex !== null
                  ? compareState.appliedHunks.get(r.hunkIndex)
                  : undefined;
              const isCurrent = r.hunkIndex === currentHunk;
              const isHunkHeader =
                r.leftType === "header" && r.leftText.startsWith("@@");
        ...
        >
          <div style={{ minHeight: "fit-content" }}>
            {rows.map((r, i) => {
              const isHunkStart =
                r.hunkIndex !== null && hunkFirstRows.get(r.hunkIndex) === i;
              const applied =
                r.hunkIndex !== null
                  ? compareState.appliedHunks.get(r.hunkIndex)
                  : undefined;
                      ...
                      style={{
                        ...
                        background:
                          applied === "left"
                            ...
                            : "var(--bg-tertiary)",
                        color:
                          applied === "left" ? "#fff" : "var(--text-secondary)",
                        border:
                          applied === "left"
                            ? "none"
              ...
              const isSep =
                r.leftType === "separator" || r.rightType === "separator";
              return (
                <div key={i} style={{ height: isSep ? ROW_H + 8 : ROW_H }} />
              );
            })}
          </div>
        </div>
