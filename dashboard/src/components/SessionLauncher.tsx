...
function TaskModal({
  ...
  const [sessionRufloPrompt, setSessionRufloPrompt] = useState<string | null>(
    null,
  );
  ...
  const effectiveTask =
    task.trim() || "Start up and ask me what I want you to do and NOTHING ELSE";
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <div
            ...
            style={{
              ...
              borderColor: mode === "agent" ? "#ef4444" : "#60a5fa",
              borderWidth: "1px",
            }}
          >
            {mode === "hivemind" ? (
              <>
                ...
                <div
                  ...
                  style={{ color: "var(--text-secondary)" }}
                >
                  Hive Mind launches a queen-led swarm that coordinates multiple
                  specialized agents working in parallel. Best for large
                  features, refactors, or complex tasks spanning multiple files.
                </div>
              </>
            ) : (
              <>
                ...
                <div
                  ...
                  style={{ color: "var(--text-secondary)" }}
                >
                  Spawns one focused agent with a specific skill set. Best for
                  targeted tasks like code review, testing, security auditing,
                  or documentation where you want deep expertise in a single
                  area.
                </div>
              </>
            )}
          ...
          <div>
            ...
            <div className="flex items-center justify-between mt-1.5">
              ...
              <SessionMicButton
                small
                onText={(text) =>
                  setTask((prev) => (prev ? `${prev} ${text}` : text))
                }
              />
            </div>
          </div>
...
export function SessionLauncher({
  ...
  const devcortexPending =
    devcortexInstallMutation.isPending || devcortexUninstallMutation.isPending;

  // Reinit state
  const [showReinitConfirm, setShowReinitConfirm] = useState(false);
  const [reinitConflicts, setReinitConflicts] = useState<{
  ...
  const btnBase =
    "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 border whitespace-nowrap min-w-0";
      ...
      <div className="w-full max-w-4xl mx-auto space-y-6">
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {devcortexData?.globalInstalled &&
                devcortexEligible &&
                (devcortexInstalled ? (
                  <button
                    onClick={() =>
                      devcortexUninstallMutation.mutate(project.id)
                    }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            >
              ...
              {gitData?.files && gitData.files.length > 0 && (
                ...
                >
                  {gitData.files.length} file
                  {gitData.files.length !== 1 ? "s" : ""} changed
                </p>
              )}
            </div>
            <div
        ...
        {!rufloInstalled && (
          <p
            ...
            style={{ color: "var(--text-secondary)" }}
          >
            Install RuFlo from the dashboard to enable Hive Mind and Agent
            sessions.
          </p>
        )}
        ...
        {onWebPageCreated &&
          (() => {
            const defaultUrl =
              project.default_web_url || "http://localhost:3000";
            ...
            {devcortexInstalled && (
              ...
              >
                ...
                <span className="text-xs">
                  Reinstall DevCortex hooks after re-init
                </span>
              </label>
            )}
          </ConfirmModal>
