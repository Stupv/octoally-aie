...
export const api = {
  sessions: {
    ...
    reconnect: (id: string) =>
      fetchJSON<{ ok: boolean; session: Session }>(
    ...
    discoverable: (projectPath?: string) => {
      const qs = projectPath
        ...
        : "";
      return fetchJSON<{ sessions: DiscoverableSession[] }>(
        `/sessions/discoverable${qs}`,
      );
    },
  ...
  projects: {
    ...
    rufloStatus: () => fetchJSON<RufloStatusResponse>("/projects/ruflo-status"),
    rufloCheck: (id: string) =>
      fetchJSON<{ settingsJson: boolean; claudeMd: boolean }>(
        `/projects/${id}/ruflo-check`,
      ),
    ...
    devcortexUninstall: (id: string) =>
      fetchJSON<{ ok: boolean }>(`/projects/${id}/devcortex`, {
        method: "DELETE",
      }),
  },
  files: {
    list: (path: string, showHidden?: boolean) =>
      fetchJSON<{ path: string; files: FileEntry[] }>(
        `/files?path=${encodeURIComponent(path)}${showHidden ? "&showHidden=true" : ""}`,
      ),
    read: (path: string) =>
      fetchJSON<{
        path: string;
  ...
  git: {
    ...
    show: (
      ...
      if (ignoreWhitespace) qs.set("ignoreWhitespace", "true");
      if (fullFile) qs.set("fullFile", "true");
      return fetchJSON<{ files: CommitFile[]; diff: string }>(
        `/git/show?${qs}`,
      );
    },
    ...
    ghAccounts: () => fetchJSON<{ accounts: string[] }>("/git/gh-accounts"),
    createRepo: (data: {
      path: string;
      name?: string;
      owner?: string;
  ...
  settings: {
    ...
    update: (settings: Record<string, string>) =>
      fetchJSON<{ ok: boolean; settings: Record<string, string> }>(
        "/settings",
        {
          method: "PUT",
  ...
  openFolder: (path: string) =>
    fetchJSON<{ ok: boolean }>("/open-folder", {
      method: "POST",
      body: JSON.stringify({ path }),
    }),
  openTerminal: (path: string) =>
    fetchJSON<{ ok: boolean }>("/open-terminal", {
      method: "POST",
      body: JSON.stringify({ path }),
    }),
  versionCheck: () =>
