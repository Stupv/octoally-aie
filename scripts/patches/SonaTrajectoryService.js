...
export class SonaTrajectoryService {
  ...
  async initialize(options) {
    ...
    try {
      ...
      if (SonaEngine && typeof SonaEngine === "function") {
        ...
        console.log(
          `[SonaTrajectoryService] Using native @ruvector/sona (dim=${this.hiddenDim})`,
        );
        return true;
      }
      console.warn(
        "[SonaTrajectoryService] @ruvector/sona loaded but SonaEngine class not found",
      );
    ...
    if (this.sona && typeof this.sona.beginTrajectory === "function") {
      try {
        ...
        const queryEmbedding =
          firstState?.embedding || this.stateToEmbedding(firstState);
        ...
        for (const step of steps) {
          ...
          const stepEmbedding =
            step.state?.embedding || this.stateToEmbedding(step.state);
        ...
        console.warn(
          `[SonaTrajectoryService] Native trajectory recording failed: ${err?.message || err}`,
        );
      }
    }
  ...
  async getPatterns(agentType) {
    ...
    if (this.sona && typeof this.sona.findPatterns === "function") {
      try {
        ...
        const neutralEmbedding = new Array(this.hiddenDim).fill(
          1.0 / Math.sqrt(this.hiddenDim),
        );
  ...
  getStats() {
    const base = {
      ...
      trajectoryCount: Array.from(this.trajectories.values()).reduce(
  ...
  sampleExperience(batchSize) {
    ...
    const totalPriority = this.experienceBuffer.reduce(
      (sum, exp) =>
        sum + Math.pow(exp.priority, this.replayConfig.priorityAlpha),
      0,
    );
    const batch = [];
  ...
  async transferLearning(sourceAgent, targetAgent, transferRatio = 0.7) {
    ...
    for (const [actionKey, weights] of this.policyWeights) {
      if (actionKey.startsWith(sourceAgent)) {
        ...
        for (
          let i = 0;
          i < Math.min(weights.length, targetWeights.length);
          i++
        ) {
          targetWeights[i] =
            transferRatio * weights[i] + (1 - transferRatio) * targetWeights[i];
        }
    ...
    for (const [stateKey, value] of this.valueWeights) {
      ...
      if (state.agentType === sourceAgent) {
        ...
        this.valueWeights.set(
          targetKey,
          transferRatio * value + (1 - transferRatio) * targetValue,
        );
      }
    }
    return true;
  ...
  async continuousLearn(state, action, reward, nextState) {
    ...
    const tdError = Math.abs(
      reward - (this.valueWeights.get(JSON.stringify(state)) || 0),
    );
    ...
    this.metrics.avgReward =
      (this.metrics.avgReward * this.metrics.iterationCount + reward) /
